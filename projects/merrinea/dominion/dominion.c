...
int baronEffect(int choice1, struct gameState *state, int currentPlayer, int handPos) {
	...
	if (choice1 > 0) { //Boolean true or going to discard an estate
		...
		int isCardActive = 1;//Flag for discard set!
		//int isCardActive = 0; bug!
		while (isCardActive) {
...
int discardHand(int handPos, int player, struct gameState *state, int numCardsInHand) {
	int p;
	for (p = 0; p < numCardsInHand; p++)
	{
		state->discard[player][state->discardCount[player]] = state->hand[player][p];
...
int drawNCards(int player, struct gameState *state, int numCardsToDraw) {
...
int minionHelper(int handPos, int player, struct gameState *state, int numCardsInHand, int numCardsToDraw) {
	discardHand(handPos, player, state, numCardsInHand);
	drawNCards(player, state, numCardsToDraw);
	return 0;
}
...
int minionEffect(int choice1, int choice2, struct gameState *state, int currentPlayer, int handPos) {
	...
	else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
		...
		int currPlayerNumCards = state->handCount[currentPlayer];
		...
		for (int i = 0; i < state->numPlayers; i++)
		{
			if (i != currentPlayer)
			{
				...
				{
					//discard hand and draw 4 cards
					int otherPlayerNumCards = state->handCount[i];
					minionHelper(handPos, i, state, otherPlayerNumCards, numDraw);
				}
			}
		}
	}
...
int tributeEffect(struct gameState* state, int currentPlayer, int nextPlayer, int handPos) {
	...
	for (int i = 0; i < 2; i++) {
		...
		else if (isTreasureCard(tributeRevealedCards[i]) == 1) { //Treasure cards
		
			state->coins += 2;
		
		}

		else if (isVictoryCard(tributeRevealedCards[i]) == 1) { //Victory cards
...
int mineEffect(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos) {
	...
	if (!(isTreasureCard(choice1)))
	{
		return -1;
	}

	if (!(isTreasureCard(choice2)))
	{
		return -1;
	}
	...
	for (int i = 0; i < state->handCount[currentPlayer]; i++)
	{
		...
		{
			discardCard(i, currentPlayer, state, 1); //refactor to trash flag
			break;
		}
	}
	return 0;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case baron:
        return baronEffect(choice1, state, currentPlayer, handPos);
    ...
    case tribute:
        return tributeEffect(state, currentPlayer, nextPlayer, handPos);
